		...
		<view class="container">
			...
			<view class="detail-content">
				<view class="price detail-list-item priceFavor"
					...
					<!-- 名字-->
					<view :style='{"textAlign":"left","fontSize":"45upx","color":"black"}'>{{detail.wenjianName}}
					</view>
				...
				<view class="detail-list-item"
					:style='{"boxShadow":"0 0 4rpx 0 #006633 insta","margin":"0 60rpx 0 60rpx","borderColor":"rgba(218, 218, 218, 1)","backgroundColor":"#fff","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"68rpx"}'
					@tap="download(detail.wenjianFile)">
					<view class="lable"
						:style='{"width":"auto","fontSize":"28rpx","color":"#f06f4f","textAlign":"left"}'>
						文件：</view>
					<view class="text" :style='{"color":"#f06f4f","textAlign":"center","fontSize":"24rpx"}'>
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.wenjianContent"></rich-text>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('wenjian', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.wenjianPhoto ? this.detail.wenjianPhoto.split(",") : [];
			},
			// 收藏
			async getwenjianCollection() {
				let params = {
					page: 1,
					limit: 1,
					wenjianId: this.id,
				...
				let res = await this.$api.list(`wenjianCollection`, params);
			...
			async shoucang() {
				...
				let params = {
					page: 1,
					limit: 1,
					wenjianId: _this.detail.id,
					yonghuId: _this.user.id,
					wenjianCollectionTypes: '1'
				}
				let res = await _this.$api.list(`wenjianCollection`, params);
				if (res.data.list.length == 1) {
					let wenjianCollectionId = res.data.list[0].id;
					uni.showModal({
						title: '提示',
						content: '是否取消',
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.del('wenjianCollection', JSON.stringify([
									wenjianCollectionId
								]));
								_this.$utils.msg('取消成功');
								_this.getwenjianCollection();
							}
						}
					});
					return;
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.add('wenjianCollection', {
								yonghuId: _this.user.id,
								wenjianId: _this.detail.id,
								wenjianCollectionTypes: 1
							});
			...
			async getThumbsup() {
				...
				if (res.data.list.length > 0) {
					if (res.data.list[0].wenjianCollectionTypes == '2') {
						_this.thumbsupFlag = true;
					}
					if (res.data.list[0].wenjianCollectionTypes == '3') {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								...
								await _this.$api.update('wenjian', _this.detail);
								_this.$utils.msg('取消成功');
